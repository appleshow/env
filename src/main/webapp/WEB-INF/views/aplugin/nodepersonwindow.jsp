...
<script type="application/javascript">
    var __nodeDataSource = function (options) {
        ...
        this._dataType = 'nodePersonWindow';
    };
    var __nodePersonWindowPar = {
        loaded: false,
        nodePersonWindowCallBack: undefined,
        currentTreeData: [],
    };
                    ...
                    if (options.id != null) {
                        if (options.type === "folder") {
                            if (options.id == "所有") {
                                ...
                                $.each(__nodePersonWindowPar.currentTreeData, function (indexNode, node) {
                                ...
                                if (options.isEnterprise) {
                                    ...
                                    $.each(__nodePersonWindowPar.currentTreeData, function (index, value) {
                        ...
                        $.ajax({
                                ...
                                success: function (res) {
                                    if (res.code != 0) {
                                        __nodePersonWindowPar.currentTreeData = [];
                                        callback({
                                            data: []
                                        });
                                    } else {
                                        ...
                                        __nodePersonWindowPar.currentTreeData = res.data;
                                        $.each(__nodePersonWindowPar.currentTreeData, function (index, value) {
                                            if (value.nodeId && value.nodeId != "") {
                                                allCount++;
                                            }
     ...
     *初始化
     */
    function __initNodePersonWidinow() {
        $('#__windowNode').modal({
            backdrop: 'static',
            keyboard: true,
            show: false,
    ...
    function __nodeSelected() {
        ...
        if (selectItems && selectItems.length > 0 && __nodePersonWindowPar.nodePersonWindowCallBack != undefined) {
            __nodePersonWindowPar.nodePersonWindowCallBack(selectItems, __nodePersonWindowPar.currentTreeData);
     ...
     * @private
     */
    function __showNodePersonWindow(callBackFunction) {
        if (!__nodePersonWindowPar.loaded) {
            __initNodePersonWidinow();

            try {
                if (callBackFunction && typeof (eval(callBackFunction)) == "function") {
                    __nodePersonWindowPar.nodePersonWindowCallBack = callBackFunction;
                }
            } catch (e) {
            }
            __nodePersonWindowPar.loaded = true;
        }
